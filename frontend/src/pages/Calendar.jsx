...
function DayView({ day, posts, onOpen, onMove, onDragOver, onDragEnd, dropTarget, busyPostId }) {
...
function WeekView({ start, postsByDay, onOpen, onMove, onDragOver, onDragEnd, dropTarget, busyPostId }) {
...
function MonthView({ cursor, start, postsByDay, onOpen, onMove, onDragOver, onDragEnd, dropTarget, busyPostId }) {
...
function DropArea({ targetKey, active, onDragOver, onDrop, className, children }) {
  return (
    <div
      className={`${className} transition ${active ? 'bg-brand-100/70 ring-2 ring-inset ring-brand-400 dark:bg-brand-900/30' : ''}`}
      onDragOver={(event) => {
...
function PostCard({ post, compact = false, detailed = false, onOpen, onDragEnd, busy = false }) {
  ...
  return (
    <button
      ...
      className={`w-full rounded-lg border border-brand-100 bg-brand-50/80 text-left text-slate-700 transition hover:border-brand-300 hover:bg-brand-100/80 focus:outline-none focus:ring-2 focus:ring-brand-500 dark:border-brand-900/60 dark:bg-brand-900/20 dark:text-slate-200 dark:hover:border-brand-700 ${
...
function PostDetailsModal({ post, action, onClose, onSave, onDelete }) {
  const [scheduledAt, setScheduledAt] = useState(() => toLocalDateTimeInput(post.scheduled_at))
  const [confirmDelete, setConfirmDelete] = useState(false)
  const editable = canReschedule(post)
  const deletable = post.status !== 'publishing'
      ...
      <div className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900" role="dialog" aria-modal="true" aria-labelledby="post-details-title">
        <div className="flex items-start justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-800">
          <div className="min-w-0 pr-4">
            ...
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Post #{post.id}</p>
          </div>
        ...
        <div className="space-y-5 p-5">
          ...
          <form onSubmit={submit} className="space-y-3 rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
            ...
            {!editable && (
              <p className="text-xs text-amber-600 dark:text-amber-400">Published or currently publishing posts cannot be rescheduled.</p>
            )}
