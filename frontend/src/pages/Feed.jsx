...
import { ChevronDown, Edit3, ExternalLink, Grid3X3, ListFilter, Plus, RefreshCw, Rss, Search, Table2, Trash2, X } from 'lucide-react'
...
export default function Feed() {
  ...
  }, [filters.category, filters.country, filters.feed_id, filters.search])
  ...
  const { hasMore, items: pagedItems, sentinelRef } = useInfiniteList(items || [])
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden">
        ...
        {items === null ? (
          ...
          <FeedTable items={pagedItems} onOpen={setSelectedItem} />
        ) : (
          <div className="columns-1 gap-4 p-4 md:columns-2 xl:columns-3">
            {pagedItems.map((item) => <FeedCard key={item.id} item={item} onOpen={setSelectedItem} />)}
          </div>
        )}
        {hasMore && <div ref={sentinelRef} className="px-4 pb-5 text-center text-xs font-semibold text-slate-400">Loading more feed items...</div>}
      </Card>
...
function PrebuiltFeedModal({ open, feeds, installedUrls, busyFeedId, onClose, onAdd, onDelete, onEdit, onRefresh }) {
  ...
  return (
    <Modal open={open} title="Add feed" description="Choose a curated source or manage the feeds already connected to this workspace." onClose={onClose} size="lg">
      <div className="space-y-4 p-5">
        <div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Connected feeds</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">{feeds.length} saved RSS source{feeds.length === 1 ? '' : 's'}</p>
            </div>
            <Badge color={feeds.some((feed) => feed.status === 'active') ? 'emerald' : 'gray'}>
              {feeds.filter((feed) => feed.status === 'active').length} active
            </Badge>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {visibleFeeds.length === 0 ? (
            ...
            return (
              <div key={feed.url} className="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
                <div className="mb-2 flex flex-wrap gap-2"><Badge color="sky">{feed.category}</Badge><Badge>{feed.country}</Badge></div>
                <p className="font-semibold text-slate-900 dark:text-white">{feed.title}</p>
                <p className="mt-1 truncate text-xs text-slate-400">{feed.url}</p>
                <Button className="mt-3 w-full" size="sm" variant={installed ? 'secondary' : 'primary'} loading={busyFeedId === feed.url} disabled={installed || busyFeedId === feed.url} onClick={() => onAdd(feed)}>
                  {installed ? 'Added' : 'Add feed'}
                </Button>
              </div>
            )
          })}
        </div>
        {feeds.length > 0 && <p className="text-center text-xs text-slate-400">{feeds.length} feed source{feeds.length === 1 ? '' : 's'} connected.</p>}
      </div>
    </Modal>
  )
}
...
function FeedItemModal({ item, onClose }) {
  return (
    <Modal open={Boolean(item)} title={item?.title || 'Feed item'} description={item ? `${item.source || item.feed?.title || 'RSS feed'} - ${formatDate(item.published_at || item.created_at)}` : ''} onClose={onClose} size="lg" fullscreenable>
      {item && (
        <div className="space-y-4 p-5">
          {item.image_url && <img src={item.image_url} alt="" className="max-h-80 w-full rounded-2xl object-cover" />}
          <div className="flex flex-wrap gap-2"><Badge color="sky">{item.category || 'General'}</Badge><Badge>{item.country || 'Global'}</Badge></div>
