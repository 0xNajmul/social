...
function RedditPreview({ account, content, media, options }) {
  const type = options?.reddit_post_type || 'self'
  const subreddit = options?.subreddit || 'community'
  const title = options?.reddit_title || 'Your Reddit title'

  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
      <div className="flex">
        <div className="flex w-10 shrink-0 flex-col items-center gap-1 bg-slate-50 py-3 text-slate-400 dark:bg-slate-800/70">
          <ThumbsUp className="h-4 w-4" />
          <span className="text-xs font-bold">1</span>
        </div>
        <div className="min-w-0 flex-1 p-3">
          <p className="text-[10px] text-slate-500">
            <span className="font-semibold text-slate-700 dark:text-slate-300">r/{subreddit}</span> · Posted by {account.username || account.name}
          </p>
          <h4 className="mt-1 text-sm font-semibold leading-5 text-slate-900 dark:text-white">{title}</h4>
          {options?.nsfw && <span className="mr-1 mt-2 inline-block rounded border border-rose-500 px-1 text-[9px] font-bold text-rose-500">NSFW</span>}
          {options?.spoiler && <span className="mt-2 inline-block rounded border border-slate-400 px-1 text-[9px] font-bold text-slate-500">SPOILER</span>}
          {type === 'image' && <MediaBlock media={media} aspect="aspect-[4/3]" />}
          {type === 'self' && (
            <p className="mt-2 line-clamp-4 whitespace-pre-wrap text-xs leading-5 text-slate-700 dark:text-slate-300">
              {content || 'Text post body…'}
            </p>
          )}
          {type === 'link' && <p className="mt-2 truncate text-xs text-blue-600">{options?.reddit_url || 'https://example.com'}</p>}
