...
export default function Workspaces() {
  ...
  const load = (query = search) => api.get('/admin/workspaces', { params: { search: query || undefined, per_page: 100 } }).then(({ data }) => setWorkspaces(data.data))
  useEffect(() => {
    api.get('/admin/workspaces', { params: { per_page: 100 } }).then(({ data }) => setWorkspaces(data.data))
    api.get('/admin/users', { params: { per_page: 100 } }).then(({ data }) => setUsers(data.data))
    api.get('/admin/plans').then(({ data }) => setPlans(data.data))
  }, [])
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold text-white">Workspaces</h1><p className="mt-1 text-sm text-slate-400">Manage ownership, plans, trials, and workspace lifecycle.</p></div><Button onClick={openCreate}><Plus className="h-4 w-4" /> Add workspace</Button></div>
      {message && <Notice message={message} />}
      <div className="flex items-center gap-2"><div className="relative max-w-md flex-1"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-500" /><Input value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && load()} className="pl-9" placeholder="Search workspaces..." /></div><Button variant="secondary" onClick={() => load()}>Search</Button></div>
