...
export default function Workspaces() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!workspaces ? <PageLoader /> : <Card className="overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[950px] text-xs"><thead className="border-b border-slate-800 bg-slate-800/30 text-left uppercase text-slate-500"><tr><th className="px-3 py-2">Workspace</th><th className="px-3 py-2">Owner</th><th className="px-3 py-2">Members</th><th className="px-3 py-2">Accounts</th><th className="px-3 py-2">Posts</th><th className="px-3 py-2">Plan</th><th className="px-3 py-2">Status</th><th className="px-3 py-2 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-800">{workspaces.map((workspace) => <tr key={workspace.id} className="text-slate-300 transition hover:bg-slate-800/35"><td className="px-3 py-2"><div className="flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-xl font-bold text-white" style={{ backgroundColor: workspace.brand_color || '#4f46e5' }}>{workspace.name?.[0]}</span><div><p className="font-medium text-white">{workspace.name}</p><p className="text-[11px] text-slate-500">{workspace.slug}</p></div></div></td><td className="px-3 py-2"><p className="text-slate-200">{workspace.owner?.name || 'Unknown'}</p><p className="text-[11px] text-slate-500">{workspace.owner?.email}</p></td><td className="px-3 py-2">{workspace.members_count}</td><td className="px-3 py-2">{workspace.social_accounts_count}</td><td className="px-3 py-2">{workspace.posts_count ?? 0}</td><td className="px-3 py-2">{workspace.subscription?.plan?.name || '—'}</td><td className="px-3 py-2">{workspace.on_trial ? <Badge color="amber">Trial</Badge> : <Badge color="emerald">Active</Badge>}</td><td className="px-3 py-2"><div className="flex justify-end gap-1.5"><Button size="sm" variant="secondary" className="px-2 py-1 text-xs" onClick={() => openEdit(workspace)}><Edit3 className="h-3.5 w-3.5" /> Edit</Button><Button size="sm" variant="ghost" className="px-2 py-1 text-rose-400 hover:bg-rose-950/30" onClick={() => setConfirmDelete(workspace)}><Trash2 className="h-3.5 w-3.5" /></Button></div></td></tr>)}</tbody></table></div></Card>}

      <Modal open={Boolean(editing)} title={editing === 'new' ? 'Create workspace' : `Edit ${editing?.name || 'workspace'}`} description="Assign ownership, plan access, and workspace defaults." onClose={() => setEditing(null)}><form onSubmit={save} className="grid gap-5 p-5 sm:grid-cols-2"><Input label="Workspace name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} error={errors.name?.[0]} required /><Input label="Timezone" value={form.timezone} onChange={(event) => setForm({ ...form, timezone: event.target.value })} error={errors.timezone?.[0]} required /><Select label="Owner" value={form.owner_id} onChange={(value) => setForm({ ...form, owner_id: value })} options={users.map((user) => [user.id, `${user.name} (${user.email})`])} error={errors.owner_id?.[0]} required /><Select label="Plan" value={form.plan_id} onChange={(value) => setForm({ ...form, plan_id: value })} options={plans.map((plan) => [plan.id, plan.name])} error={errors.plan_id?.[0]} required />{editing === 'new' ? <Input label="Trial days" type="number" min="0" max="365" value={form.trial_days} onChange={(event) => setForm({ ...form, trial_days: event.target.value })} error={errors.trial_days?.[0]} /> : <><DateTimeField label="Trial ends at" type="datetime-local" value={form.trial_ends_at} onChange={(event) => setForm({ ...form, trial_ends_at: event.target.value })} error={errors.trial_ends_at?.[0]} /><label className="block"><span className="mb-1.5 block text-sm font-medium text-slate-300">Brand color</span><div className="flex gap-3"><input type="color" value={form.brand_color} onChange={(event) => setForm({ ...form, brand_color: event.target.value })} className="h-11 w-16 rounded-xl border border-slate-700 bg-slate-800 p-1" /><Input value={form.brand_color} onChange={(event) => setForm({ ...form, brand_color: event.target.value })} /></div></label></>}<div className="flex justify-end gap-2 border-t border-slate-800 pt-4 sm:col-span-2"><Button type="button" size="sm" variant="ghost" onClick={() => setEditing(null)}>Cancel</Button><Button type="submit" size="sm" loading={busy}><Building2 className="h-4 w-4" /> Save workspace</Button></div></form></Modal>
